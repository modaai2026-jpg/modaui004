...
export default function KnowledgeBaseView({ onBackToLanding }: { onBackToLanding: () => void }) {
  ...
  const fetchFiles = async () => {
    try {
      const industryId = localStorage.getItem('preview_industry_id') || 'fashion';
      const res = await fetch(`/api/knowledge?tenantId=${tenantId}&industryId=${industryId}`);
  ...
  const handleRemoveFile = async (id: string) => {
    try {
      if (id.startsWith('chk_')) {
        const industryId = localStorage.getItem('preview_industry_id') || 'fashion';
        await fetch(`/api/knowledge/${id}?tenantId=${tenantId}&industryId=${industryId}`, {
          method: 'DELETE'
        });
      }
